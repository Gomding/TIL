...
코드 계층과 클래스 생성과 관련한 네 가지 핵심 요소
- **코드 가독성**: 단일 클래스에 담겨 있는 개념이 많을 수록 해당 클래스의 가독성은 저하됨. 개념이 많아 인지적 부담이 많아지고 읽는 시간이 길어진다. 코드를 잘못 이해할 가능성이 커진다.
- **코드 모듈화**: 클래스 및 인터페이스의 사용은 코드 모듈화를 위한 좋은 방법 중 하나다. 추상화를 잘 해냈다면 하위 문제에 대한 해결책의 구현은 쉽게 교체할 수 있다.
- **코드 재사용성 및 일반화**: 계층을 잘 나눠서 하위 문제를 해결했다면 나중에 다른 문제의 하위 문제를 재사용할 가능성이 꽤 높다.
- **테스트 용이성 및 적절한 테스트**: 로직이 여러 클래스로 나누어지면 각 부분을 적절하게 테스트하기가 훨씬 쉬워진다.

클래스 내부에서 여러 함수로 나누어 추상화 계층을 만들었다고 하여 클래스가 적절히 분리된 것은 아니다.
- 여전히 하나의 클래스가 모든 것을 다 가짐
- 이는 추상화 계층들 사이의 분리가 그다지 뚜렷하지 않은것을 의미
- 관련 하위 문제를 해결하는 것이 원래 문제와는 다른 관심사인지 아니면 본질적으로 원래 문제의 일부분으로 간주하여야 하는지에 대해서는 의견이 다르다.
- 클래스가 분리되어야 할지 판단을 위해 네 가지 핵심 요소에 반하여 작성되어 있는지 확인
  - 코드를 읽을 수 없다.
  - 코드가 특별히 모듈화되어 있지 않다.
  - 코드를 재사용할 수 없다.
  - 코드를 일반화할 수 없다.
...
#### 인터페이스
- 계층 사이를 뚜럿이 구분하고 구현 세부 사항이 계층 사이에 유출되지 않도록 하기 위해 사용할 수 있는 접근법 **인터페이스**
- 어떤 함수를 외부로 노출할 것인지를 인터페이스를 통해 결정하는 것
- 상위 계층은 인터페이스에 의존할 뿐 로직을 구현하는 구체적인 클래스에 의존하지 않는다. (DIP)

#### 모든 것을 위한 인터페이스
- 주어진 추상화 계층에 한 가지 구현만 있고 향후에 다른 구현을 추가할 계획이 없더라도 여전히 인터페이스를 통해 추상화 계층을 표현해야 하는가?
- 즉, 모든 것에 인터페이스를 만들어서 사용해야 하는가?
- 장점
...
모든 클래스에 인터페이스를 붙이는 극단적인 코드는 종종 통제가 불가능하고 불필요하게 복잡해지며 이해와 수정이 어려워진다.

장점이 확실한 상황에서는 인터페이스를 사용하되, 인터페이스를 위한 인터페이스를 작성해서는 안된다.

(인터페이스를 정의하지 않더라도 클래스에서 어떤 함수를 퍼블릭으로 노출할지는 매우 신중하게 생각해야 한다. 구현 세부사항이 유출되지 않도록 해야 한다.)

#### 층이 너무 얇아질 때
코드를 별개의 계층으로 세분화하면 장점이 많지만 다음과 같은 추가 비용이 발생
- 구현(implement) 를 위해 반복적인 코드의 양이 증가한다. (보일러 플레이트 코드)
- 로직의 이해를 위해 파일이나 클래스를 따라갈 때 더 많은 노력이 필요
- 인터페이스의 어떤 구현이 사용되는지 파악하는 데 더 많은 노력이 필요하다.

**무지성 분할은 단점이 생긴다! 장점이 생기는지 생각해보고 인터페이스를 분리하자**

계층이 너무 두꺼운지 여부를 결정해주는 규칙이나 정답은 존재하지 않는다.
- 계층을 몇개로 나눌지는 해결하려고 하는 실제 현실의 문제에 달려있다.
- 자신이 만든 계층이 코드의 가독성을 높이고, 일반화하여 재사용성이 증가하고, 모듈화 가능하고, 테스트를 용이하게 만드는지를 스스로 판단해야한다.
- 추상화 계층을 올바르게 만들기 위해 코드의 설계와 재작업을 여러 번 반복해야 할 수도 있다는 점을 잊지 말자!
